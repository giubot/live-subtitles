...
| OUT-5b | **Step-by-step guide** (`docs/manual/obs-vmix-guide.md`, with screenshots) for adding the overlay: **OBS** (Sources → Browser → URL, 1920×1080, no custom CSS, layer above the video source) and **vMix** (Add Input → Web Browser → URL, 1920×1080, place it as an overlay channel or layer over the program input; the page background is transparent, so no keying is needed). It also covers multi-language streams (one overlay per language or scene) and troubleshooting. | M |
