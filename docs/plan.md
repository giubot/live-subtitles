...
| **P0-04** | **Web skeleton** | web-shell | P0-01 | UI-1, UI-2, UI-3, UI-5, UI-7 | Vite + React + TS; TanStack Router (file routes) with **stub routes for every page**: `/setup`, `/admin/*`, `/capture/$id`, `/s`, `/s/$id`, `/stage/$id`, `/overlay/$id`, `/replay/$id`; TanStack Query provider; Zustand; MUI theme with `colorSchemes` + **Light/Dark/System** toggle (no flash); **i18next ES/EN** with browser detection, `?ui=` override, switcher, MUI locale switch, per-feature namespaces; bare overlay layout (transparent, no theme); ESLint + Prettier + Vitest; missing-i18n-key check in `task check`. |
